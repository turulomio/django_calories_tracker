...
def test_elaborations_experiences(self):     
    ...
    dict_recipes_berfore=tests_helpers.client_get(self, self.client_authorized_1, dict_recipes["url"], status.HTTP_200_OK)
    dict_elaborations_experience=tests_helpers.client_post(self, self.client_authorized_1, "/api/elaborations_experiences/", models.ElaborationsExperiences.post_payload(elaborations=dict_elaborations["url"]), status.HTTP_201_CREATED)
    dict_recipes_after=tests_helpers.client_get(self, self.client_authorized_1, dict_recipes["url"], status.HTTP_200_OK)
    self.assertNotEqual(dict_recipes_berfore["last"], dict_recipes_after["last"])   